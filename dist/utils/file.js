...
const uploadDir = process.env.FILE_UPLOAD_DIR || _path2.default.join(__dirname, 'upload');
...
const storage = _multer2.default.diskStorage({
    ...
    filename(req, file, cb) {
        ...
        const fileobj = new _File2.default({
...
// 파일 업로드 multer 미들웨어
// 파일 외부에서의 직접 접근을 막는다.
const rawUpload = (0, _multer2.default)({ storage });

// 업로드된 파일의 정보를 db에 업데이트 하기위한 미들웨어
const upload = exports.upload = {
    single(field) {
        return (req, res, next) => {
            ...
            const afterUpload = () => {
                ...
                _File2.default.findById(req.file.filename).then(file => {
...
async function getFileInfo(file_id) {
    const file = await _File2.default.findById(file_id).cache();
...
async function getFileInfoArray(files) {
    const output = [];
...
function getFilePath(file_id) {
    return _path2.default.join(uploadDir, file_id);
}
 ...
 * @param {ObjectID} file_id
 */
async function increaseFileHit(file_id) {
    const file = await _File2.default.findById(file_id).cache();
...
function setUploadExpireTimeout(fileId, timeout) {
    setTimeout(async () => {
        const file = await _File2.default.findById(fileId);
        if (!file.hasLink()) {
 ...
 * @param {*} fileId 파일의 ID
 */
async function checkAttachableFile(fileId) {
    // let file
    try {
        await _File2.default.findById(fileId + '');
    } catch (error) {
 ...
 */
async function checkAttachableFileArray(files) {
 ...
 * @param {*} ref
 */
async function applyFileLink(files, target, ref) {
    if (!Array.isArray(files)) {
        files = [files];
    }
    for (let fileId of files) {
 ...
 * @param {*} files
 */
async function removeFileLink(files) {
    if (!Array.isArray(files)) {
        files = [files];
    }
    for (let fileId of files) {
 ...
 * @param {*} files
 */
async function deleteUnlinkedFile(files) {
    if (!Array.isArray(files)) {
        files = [files];
    }
    for (let fileId of files) {
...
async function cleanupUnlinkedFiles() {
    const allFiles = await _File2.default.find();
    await deleteUnlinkedFile(allFiles.map(file => {
        return file.id;
    }));
}
...
async function deleteFile(fileId) {
    try {
        const file = await _File2.default.findById(fileId);
        try {
            await _fs2.default.promises.unlink(_path2.default.join(uploadDir, file.id));
        } catch (error) {
            //
        }
 ...
 */
async function checkIsFileOwner(files, username) {
    if (!Array.isArray(files)) {
        files = [files];
    }
    for (let fileId of files) {
 ...
 * @param {*} files
 */
async function checkUnlinkedFile(files) {
    if (!Array.isArray(files)) {
        files = [files];
    }
    for (let fileId of files) {
...
async function getFileLinks(files) {
    if (!Array.isArray(files)) {
        files = [files];
    }
    const res = [];
