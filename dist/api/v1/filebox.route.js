...
var _Group = require('../../models/filebox/Group');

var _Group2 = _interopRequireDefault(_Group);

var _Material = require('../../models/filebox/Material');
...
router.get('/', [], (0, _api.asyncRoute)(async (req, res) => {
    ...
    const loops = async item => {
        const res = await _Group2.default.find().where('parent').equals(item.id).sort('isfolder');
    ...
    const roots = await _Group2.default.find().exists('parent', false).sort('isfolder');
 ...
 */
router.post('/group', [(0, _expressValidator.body)('name').isString(), (0, _expressValidator.body)('isfolder').isBoolean(), (0, _expressValidator.body)('parent_id').isMongoId().optional(), (0, _role.perm)('fileBox').can('manage'), _api.validateParams], (0, _api.asyncRoute)(async (req, res) => {
    let parent;
    //부모 id가 없다면 첫 번째 group
    if (req.body.parent_id) {
        parent = await _Group2.default.findById(req.body.parent_id);
    ...
    let newGroup = new _Group2.default();
 ...
 * @apiPermission can.manage
 */
router.patch('/group/:group_id', [(0, _expressValidator.param)('group_id').isMongoId(), (0, _expressValidator.body)('name').isString(), (0, _expressValidator.body)('isfolder').isBoolean(), (0, _expressValidator.body)('parent_id').isMongoId().optional(), (0, _role.perm)('fileBox').can('manage'), _api.validateParams], (0, _api.asyncRoute)(async function (req, res) {
    let group = await _Group2.default.findById(req.params.group_id);
    if (group) {
 ...
 * @apiPermission can.manage
 */
router.delete('/group/:group_id', [(0, _role.perm)('fileBox').can('manage'), (0, _expressValidator.param)('group_id').isMongoId(), _api.validateParams], (0, _api.asyncRoute)(async (req, res) => {
    // 트리 재귀적 삭제 루프
    const loops = async group_id => {
        const group = await _Group2.default.findById(group_id);
        if (!group) {
        ...
        if (group.parent) {
            const parent = await _Group2.default.findById(group.parent);
            if (parent) {
        ...
        if (group.isfolder) {
            const materials = await _Material2.default.find().where('parent').equals(group_id);

            for (let material of materials) {
                await (0, _file.removeFileLink)(material.files);
                await (0, _file.deleteUnlinkedFile)(material.files);
 ...
 *       }
 */
router.get('/folder/:parent_id', [(0, _expressValidator.param)('parent_id').isMongoId(), _api.validateParams], (0, _api.asyncRoute)(async (req, res) => {
    const folder = await _Group2.default.findById(req.params.parent_id);
    if (!folder) {
    ...
    const materials = await _Material2.default.find().where('parent').equals(req.params.parent_id).sort('-_id');
    ...
    for (let item of materials) {
        mat.push({
            ...
            files: await (0, _file.getFileInfoArray)(item.files)
        });
    }
    res.json({
        folder: {
 ...
 */
router.post('/folder/:parent_id', [(0, _expressValidator.body)('title').isString(), (0, _expressValidator.body)('content').isString(), (0, _expressValidator.body)('files').custom(_file.checkAttachableFileArray), (0, _expressValidator.param)('parent_id').isMongoId(), _api.validateParams], (0, _api.asyncRoute)(async (req, res) => {
    if (!req.user.perm('fileBox').can('upload') && !req.user.perm('fileBox').can('manage')) {
        const err = new Error('권한이 없습니다.');
        err.status = 403;
        throw err;
    }

    const parent = await _Group2.default.findById(req.params.parent_id);
    if (!parent) {
    ...
    const newMaterial = new _Material2.default({
    ...
    // DB 파일 객체에 역참조 등록
    await (0, _file.applyFileLink)(req.body.files, 'filebox', newMaterial.id);
 ...
 *       }
 */
router.get('/material/:material_id', [(0, _expressValidator.param)('material_id').isMongoId(), _api.validateParams], (0, _api.asyncRoute)(async (req, res) => {
    const material = await _Material2.default.findById(req.params.material_id);
    ...
    res.status(200).json({
        ...
        content: material.content,
        created_date: material.created_date,
        files: await (0, _file.getFileInfoArray)(material.files)
    });
}));
 ...
 * @apiPermission can.upload
 */
router.delete('/material/:material_id', [(0, _expressValidator.param)('material_id').isMongoId(), _api.validateParams], (0, _api.asyncRoute)(async (req, res) => {
    const material = await _Material2.default.findById(req.params.material_id);
    if (!req.user.perm('fileBox').can('manage')) {
        //파일을 올린 본인이 아니라면 삭제할 수 없도록함
        if (req.user.perm('fileBox').can('upload') && material.author != req.user.username) {
 ...
 * @apiPermission can.upload
 */
router.patch('/material/:material_id', [(0, _expressValidator.param)('material_id').isMongoId(), (0, _expressValidator.body)('title').isString(), (0, _expressValidator.body)('content').isString(), (0, _expressValidator.body)('files').custom(_file.checkAttachableFileArray), _api.validateParams], (0, _api.asyncRoute)(async (req, res) => {
    const material = await _Material2.default.findById(req.params.material_id);
